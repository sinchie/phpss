...
class Encryptor
{
    ...
    protected static function merge_sort($array, $comparison)
    {
        if (count($array) < 2) {
            return $array;
        }
        $middle = ceil(count($array) / 2);
        return self::merge(self::merge_sort(self::slice($array, 0, $middle), $comparison), self::merge_sort(self::slice($array, $middle), $comparison), $comparison);
    }
